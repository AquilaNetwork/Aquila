...
if [ -z "${pid}" ]; then
  pid=$(ps aux | grep '[a]quila.jar' | head -n 1 | awk '{print $2}')
  is_pid_valid=$?
fi
...
if [ -n "$apikey" ]; then
  echo "Stopping Aquila network via API..."
...
if [ "$success" -ne 1 ] && [ -n "$pid" ]; then
  echo "Stopping Aquila network process $pid..."
  if kill -15 "${pid}"; then
    success=1
  fi
fi
...
if [ "$success" -eq 1 ]; then
  echo "Aquila node should be shutting down"
  if [ "${is_pid_valid}" -eq 0 ]; then
    echo -n "Monitoring for Aquila node to end"
    while s=`ps -p $pid -o stat=` && [[ "$s" && "$s" != 'Z' ]]; do
      echo -n .
      sleep 1
    done
    echo
    echo "${green}Aquila ended gracefully${normal}"
    rm -f run.pid
  fi
fi
