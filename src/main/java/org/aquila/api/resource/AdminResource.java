...
import org.aquila.api.model.ActivitySummary;
import org.aquila.api.model.NodeInfo;
...
import org.aquila.controller.Synchronizer.SynchronizationResult;
import org.aquila.controller.repository.BlockArchiveRebuilder;
...
public class AdminResource {
	...
	public String addMintingAccount(@HeaderParam(Security.API_KEY_HEADER) String apiKey, String seed58) {
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			// Aquila: account must derive to known reward-share public key
			...
			// Aquila: check reward-share's minting account is still allowed to mint
	...
	@Operation(
		...
		requestBody = @RequestBody(
			...
			content = @Content(
				mediaType = MediaType.TEXT_PLAIN,
				schema = @Schema(
					type = "string", example = "node2.aquila.org"
				)
			)
		),
		responses = {
	...
	@Operation(
		...
		description = "Imports data from file on local machine. Filename is forced to 'aquila-backup/TradeBotStates.json' if apiKey is not set.",
		requestBody = @RequestBody(
			required = true,
			content = @Content(
				mediaType = MediaType.TEXT_PLAIN,
				schema = @Schema(
					type = "string", example = "aquila-backup/TradeBotStates.json"
				)
			)
		),
		responses = {
