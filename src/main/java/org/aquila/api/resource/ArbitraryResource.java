...
import org.aquila.controller.arbitrary.ArbitraryMetadataManager;
import org.aquila.data.account.AccountData;
import org.aquila.data.arbitrary.*;
...
import org.aquila.transform.transaction.ArbitraryTransactionTransformer;
	...
	private String upload(Service service, String name, String identifier,
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			if (path == null) {
				...
				if (string != null) {
					File tempFile = File.createTempFile("aquila-", ".tmp");
			...
			if (zipped) {
				// Unzip the file
				java.nio.file.Path tempDirectory = Files.createTempDirectory("aquila-");
	...
	private HttpServletResponse download(Service service, String name, String identifier, String filepath, boolean rebuild, boolean async, Integer maxAttempts) {
		...
		try {
			...
			if (filepath == null || filepath.isEmpty()) {
				...
				String[] files = ArrayUtils.removeElement(outputPath.toFile().list(), ".aquila");
				if (files.length == 1) {
