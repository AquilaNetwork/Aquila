...
import org.aquila.crosschain.Litecoin;
...
public class LitecoinACCTv1TradeBot implements AcctTradeBot {
	...
	/**
	 * Creates a new trade-bot entry from the "Bob" viewpoint, i.e. OFFERing UNCIA in exchange for LTC.
	 * <p>
	 * Generates:
	 * <ul>
	 ...
	 * Derives:
	 * <ul>
	 * 	<li>'native' (as in Aquila) public key, public key hash, address (starting with Q)</li>
	 * 	<li>'foreign' (as in Litecoin) public key, public key hash</li>
	 * </ul>
	 * A Aquila AT is then constructed including the following as constants in the 'data segment':
	 * <ul>
	 ...
	 * </ul>
	 * Returns a DEPLOY_AT transaction that needs to be signed and broadcast to the Aquila network.
	 * <p>
	...
	public byte[] createTrade(Repository repository, TradeBotCreateRequest tradeBotCreateRequest) throws DataException {
		...
		String name = "UNCIA/LTC ACCT";
		String description = "UNCIA/LTC cross-chain trade";
		String aTType = "ACCT";
		String tags = "ACCT UNCIA LTC";
		byte[] creationBytes = LitecoinACCTv1.buildAquilaAT(tradeNativeAddress, tradeForeignPublicKeyHash, tradeBotCreateRequest.unciaAmount,
		...
		// Return to user for signing and broadcast as we don't have their Aquila private key
		try {
	 ...
	 * <p>
	 * It is envisaged that the value in <tt>xprv58</tt> will actually come from a Aquila-UI-managed wallet.
	 * <p>
	...
	public ResponseResult startResponse(Repository repository, ATData atData, ACCT acct, CrossChainTradeData crossChainTradeData, String xprv58, String receivingAddress) throws DataException {
		...
		// Attempt to send MESSAGE to Bob's Aquila trade address
		...
		String messageRecipient = crossChainTradeData.aquilaCreatorTradeAddress;
	...
	private void handleAliceWaitingForAtLock(Repository repository, TradeBotData tradeBotData,
		...
		List<MessageTransactionData> messageTransactionsData = repository.getMessageRepository().getMessagesByParticipants(tradeBotData.getTradeNativePublicKey(),
				crossChainTradeData.aquilaCreatorTradeAddress, null, null, null);
		if (messageTransactionsData == null || messageTransactionsData.isEmpty()) {
			LOGGER.warn(() -> String.format("Unable to find our message to trade creator %s?", crossChainTradeData.aquilaCreatorTradeAddress));
			return;
		}
		...
		String aquilaReceivingAddress = Base58.encode(tradeBotData.getReceivingAccountInfo()); // Actually contains whole address, not just PKH
		byte[] messageData = LitecoinACCTv1.buildRedeemMessage(secretA, aquilaReceivingAddress);
		String messageRecipient = tradeBotData.getAtAddress();
		...
		TradeBot.updateTradeBotState(repository, tradeBotData, State.ALICE_DONE,
				() -> String.format("Redeeming AT %s. Funds should arrive at %s",
						tradeBotData.getAtAddress(), aquilaReceivingAddress));
	}
	...
	private void handleBobWaitingForAtRedeem(Repository repository, TradeBotData tradeBotData,
		...
		if (crossChainTradeData.mode == AcctMode.REFUNDED || crossChainTradeData.mode == AcctMode.CANCELLED) {
			// Alice hasn't redeemed the UNCIA, so there is no point in trying to redeem the LTC
			TradeBot.updateTradeBotState(repository, tradeBotData, State.BOB_REFUNDED,
	...
	private boolean aliceUnexpectedState(Repository repository, TradeBotData tradeBotData,
		...
		boolean isAtLockedToUs = tradeBotData.getTradeNativeAddress().equals(crossChainTradeData.aquilaPartnerAddress);
		...
		if (!atData.getIsFinished() && crossChainTradeData.mode == AcctMode.TRADING)
			if (isAtLockedToUs) {
				...
				TradeBot.updateTradeBotState(repository, tradeBotData, State.ALICE_REFUNDING_A,
						() -> String.format("AT %s trading with someone else: %s. Refunding & aborting trade", tradeBotData.getAtAddress(), crossChainTradeData.aquilaPartnerAddress));
