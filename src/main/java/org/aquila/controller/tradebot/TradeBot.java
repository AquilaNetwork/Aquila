...
import org.aquila.controller.tradebot.AcctTradeBot.ResponseResult;
...
import org.aquila.data.network.TradePresenceData;
...
import org.aquila.network.message.GetTradePresencesMessage;
import org.aquila.network.message.Message;
import org.aquila.network.message.TradePresencesMessage;
import org.aquila.repository.DataException;
import org.aquila.repository.Repository;
import org.aquila.repository.RepositoryManager;
import org.aquila.repository.hsqldb.HSQLDBImportExport;
import org.aquila.settings.Settings;
import org.aquila.utils.ByteArray;
...
public class TradeBot implements Listener {
	...
	/*package*/ void updatePresence(Repository repository, TradeBotData tradeBotData, CrossChainTradeData tradeData)
		...
		*/
		if (!signerAddress.equals(tradeData.aquilaCreatorTradeAddress) && !signerAddress.equals(tradeData.aquilaPartnerAddress)) {
	...
	public void decorateTradeDataWithPresence(CrossChainTradeData crossChainTradeData) {
		...
		this.safeAllTradePresencesByPubkey.values().stream()
				.filter(tradePresenceData -> tradePresenceData.getAtAddress().equals(crossChainTradeData.aquilaAtAddress))
				.forEach(tradePresenceData -> {
					String signerAddress = tradePresenceData.getTradeAddress();

					// Signer's public key (in address form) must match Bob's / Alice's trade public key (in address form)
					if (signerAddress.equals(crossChainTradeData.aquilaCreatorTradeAddress))
						crossChainTradeData.creatorPresenceExpiry = tradePresenceData.getTimestamp();
					else if (signerAddress.equals(crossChainTradeData.aquilaPartnerAddress))
