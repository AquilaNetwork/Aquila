...
public class HSQLDBAccountRepository implements AccountRepository {
	...
	public AccountData getAccount(String address) throws DataException {
		String sql = "SELECT reference, public_key, default_group_id, flags, level, blocks_minted, blocks_minted_adjustment, sponsor_public_key FROM Accounts WHERE account = ?";
		...
		try (ResultSet resultSet = this.repository.checkedExecute(sql, address)) {
			...
			byte[] sponsorPublicKey = resultSet.getBytes(8);

			return new AccountData(address, reference, publicKey, defaultGroupId, flags, level, blocksMinted, blocksMintedAdjustment, sponsorPublicKey);
	...
	public List<AccountData> getFlaggedAccounts(int mask) throws DataException {
		String sql = "SELECT reference, public_key, default_group_id, flags, level, blocks_minted, blocks_minted_adjustment, sponsor_public_key, account FROM Accounts WHERE BITAND(flags, ?) != 0";
		...
		try (ResultSet resultSet = this.repository.checkedExecute(sql, mask)) {
			...
			do {
				...
				accounts.add(new AccountData(address, reference, publicKey, defaultGroupId, flags, level, blocksMinted, blocksMintedAdjustment, sponsorPublicKey));
	...
	// get Sponsor added by sahkan
	@Override
	public byte[] getSponsorPublicKey(String address) throws DataException {
		String sql = "SELECT sponsor_public_key FROM Accounts WHERE account = ?";
