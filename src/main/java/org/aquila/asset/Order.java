...
import static org.aquila.utils.Amounts.prettyAmount;
...
public class Order {
	...
	/** Cache of price-pair units e.g. UNCIA/GOLD, but use getPricePair() instead! */
	...
	/** Calculate price pair. (e.g. UNCIA/GOLD)
	 * <p>
	 * Lowest-assetID asset is first,
	 * so if UNCIA has assetID 0 and GOLD has assetID 10, then
	 * the pricing pair is UNCIA/GOLD.
	 * <p>
	...
	private void matchOrders(List<OrderData> orders) throws DataException {
		 ...
		 * 
		 * Our order:
		 * haveAssetId=[GOLD], wantAssetId=0 (UNCIA), amount=40 (GOLD), price=486 (UNCIA/GOLD)
		 * This translates to "we have 40 GOLD and want UNCIA at a price of 486 UNCIA per GOLD"
		 * If our order matched, we'd end up with 19,440 UNCIA at a cost of 19,440 / 486 = 40 GOLD.
		 * 
		 * Their order:
		 * haveAssetId=0 (UNCIA), wantAssetId=[GOLD], amount=40 (GOLD), price=486.00074844 (UNCIA/GOLD)
		 * This translates to "they have UNCIA and want GOLD at a price of 486.00074844 UNCIA per GOLD"
		 * 
		 * Their price is better than our requested 486 UNCIA/GOLD so this order matches.
		 * 
		 * Using their price, we end up with 40 * 486.00074844 = 19440.02993760 UNCIA. They end up with 40 GOLD.
		 * 
		 * If their order only had 36 GOLD left, only 36 * 486.00074844 = 17496.02694384 UNCIA would be traded.
		 */
