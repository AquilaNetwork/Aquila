...
import org.aquila.arbitrary.metadata.ArbitraryDataTransactionMetadata;
import org.aquila.arbitrary.misc.Category;
import org.aquila.arbitrary.misc.Service;
import org.aquila.crypto.AES;
...
public class ArbitraryDataWriter {
    ...
    private void validatePatch() throws DataException {
        ...
        File aquilaMetadataDirectoryFile = Paths.get(this.filePath.toString(), ".aquila").toFile();
        if (!aquilaMetadataDirectoryFile.exists()) {
            throw new DataException("Aquila metadata folder doesn't exist in patch");
        }
        if (!aquilaMetadataDirectoryFile.isDirectory()) {
            throw new DataException("Aquila metadata folder isn't a directory");
        }

        File aquilaPatchMetadataFile = Paths.get(this.filePath.toString(), ".aquila", "patch").toFile();
        if (!aquilaPatchMetadataFile.exists()) {
            throw new DataException("Aquila patch metadata file doesn't exist in patch");
        }
        if (!aquilaPatchMetadataFile.isFile()) {
            throw new DataException("Aquila patch metadata file isn't a file");
        }
    }
