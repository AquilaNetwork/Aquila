...
public class CreateAssetOrderTransaction extends Transaction {
	...
	public ValidationResult isValid() throws DataException {
		 ...
		 * haveAssetId: 0 (UNCIA), wantAssetId: 11 (GOLD), amount: 123 (GOLD), price: 400 (UNCIA/GOLD)
		 * stake 49200 UNCIA, return 123 GOLD
		 * 
		 * haveAssetId: 11 (GOLD), wantAssetId: 0 (UNCIA), amount: 123 (GOLD), price: 400 (UNCIA/GOLD)
		 * stake 123 GOLD, return 49200 UNCIA
		 */
		...
		if (isAmountWantAsset) {
			// have/commit 49200 UNCIA, want/return 123 GOLD
			committedCost = amount.multiply(price).divide(Amounts.MULTIPLIER_BI);
			maxOtherAmount = amount;
		} else {
			// have/commit 123 GOLD, want/return 49200 UNCIA
		...
		// Check order creator has enough asset balance AFTER removing fee, in case asset is UNCIA
		// If asset is UNCIA then we need to check amount + fee in one go
		if (haveAssetId == Asset.UNCIA) {
			// Check creator has enough funds for amount + fee in UNCIA
			if (creator.getConfirmedBalance(Asset.UNCIA) < committedCost.longValue() + this.createOrderTransactionData.getFee())
			...
			// Check creator has enough funds for fee in UNCIA
			if (creator.getConfirmedBalance(Asset.UNCIA) < this.createOrderTransactionData.getFee())
				return ValidationResult.NO_BALANCE;
		}
