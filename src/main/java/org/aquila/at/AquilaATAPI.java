...
import org.aquila.repository.ATRepository;
import org.aquila.repository.DataException;
import org.aquila.repository.Repository;
import org.aquila.repository.ATRepository.NextTransactionInfo;
import org.aquila.transaction.AtTransaction;
...
public class AquilaATAPI extends API {

	private static final byte[] ADDRESS_PADDING = new byte[32 - Account.ADDRESS_LENGTH];
	private static final Logger LOGGER = LogManager.getLogger(AquilaATAPI.class);
	...
	public AquilaATAPI(Repository repository, ATData atData, long blockTimestamp) {
	...
	// Methods specific to Aquila AT processing, not inherited
	...
	public long getCurrentBalance(MachineState state) {
		try {
			...
			return atAccount.getConfirmedBalance(Asset.UNCIA);
		} catch (DataException e) {
	...
	public void platformSpecificPreExecuteCheck(int paramCount, boolean returnValueExpected, MachineState state, short rawFunctionCode)
		...
		AquilaFunctionCode aquilaFunctionCode = AquilaFunctionCode.valueOf(rawFunctionCode);

		if (aquilaFunctionCode == null)
			throw new IllegalFunctionCodeException("Unknown Aquila function code 0x" + String.format("%04x", rawFunctionCode) + " encountered");

		aquilaFunctionCode.preExecuteCheck(paramCount, returnValueExpected, rawFunctionCode);
	}
	...
	public void platformSpecificPostCheckExecute(FunctionData functionData, MachineState state, short rawFunctionCode) throws ExecutionException {
		...
		aquilaFunctionCode.execute(functionData, state, rawFunctionCode);
	}
	...
	/* Convenience methods to allow AquilaFunctionCode package-visibility access to A/B-get/set methods. */
