...
/**
 * Aquila-specific CIYAM-AT Functions.
 * <p>
 * Function codes need to be between 0x0500 and 0x06ff.
 *
 */
public enum AquilaFunctionCode {
	/**
	...
	GET_MESSAGE_LENGTH_FROM_TX_IN_A(0x0501, 0, true) {
		...
		protected void postCheckExecute(FunctionData functionData, MachineState state, short rawFunctionCode) throws ExecutionException {
			AquilaATAPI api = (AquilaATAPI) state.getAPI();
	...
	/**
	 * Convert 20-byte value in LSB of B1, and all of B2 & B3 to Aquila address.<br>
	 * <tt>0x0512</tt><br>
	 * Aquila address stored in lower 25 bytes of B.
	 */
	CONVERT_B_TO_AQUILA(0x0512, 0, false) {
		@Override
		protected void postCheckExecute(FunctionData functionData, MachineState state, short rawFunctionCode) throws ExecutionException {
	 ...
	 * <tt>0x0520</tt><br>
	 * B should contain either Aquila address or public key,<br>
	 ...
	 * <p></p>
	 * @see AquilaATAPI#getAccountFromB(MachineState)
	 */
	...
	private static final Logger LOGGER = LogManager.getLogger(AquilaFunctionCode.class);

	private static final Map<Short, AquilaFunctionCode> map = Arrays.stream(AquilaFunctionCode.values())
			.collect(Collectors.toMap(functionCode -> functionCode.value, functionCode -> functionCode));

	private AquilaFunctionCode(int value, int paramCount, boolean returnsValue) {
		this.value = (short) value;
		this.paramCount = paramCount;
		this.returnsValue = returnsValue;
	}

	public static AquilaFunctionCode valueOf(int value) {
