...
import org.aquila.account.AccountRefCache;
import org.aquila.account.PrivateKeyAccount;
import org.aquila.account.PublicKeyAccount;
import org.aquila.asset.Asset;
import org.aquila.at.AT;
import org.aquila.block.BlockChain.BlockTimingByHeight;
import org.aquila.block.BlockChain.AccountLevelShareBin;
...
public class Block {
	...
	public static Block mint(Repository repository, BlockData parentBlockData, PrivateKeyAccount minter) throws DataException {
		...
		// Aquila: minter is always a reward-share, so find actual minter and get their effective minting level
		...
		if (timestamp >= BlockChain.getInstance().getAggregateSignatureTimestamp()) {
			...
			onlineAccountsSignatures = Aquila25519Extras.aggregateSignatures(signaturesToAggregate);
		} else {
	...
	public ValidationResult areOnlineAccountsValid() throws DataException {
		...
		if (this.blockData.getTimestamp() >= BlockChain.getInstance().getAggregateSignatureTimestamp()) {
			...
			if (!Aquila25519Extras.verifyAggregated(aggregatePublicKey, aggregateSignature, onlineTimestampBytes))
	...
	protected boolean isMinterValid(Block parentBlock) throws DataException {
		// Aquila: block's minter public key must be known reward-share public key
