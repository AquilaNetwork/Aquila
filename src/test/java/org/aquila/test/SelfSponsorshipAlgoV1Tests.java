...
import org.aquila.transaction.TransferPrivsTransaction;
import org.aquila.utils.NTP;
import org.junit.Before;
import org.junit.Test;
import org.aquila.test.common.*;
import org.aquila.test.common.transaction.TestTransaction;

import java.util.*;

import static org.aquila.transaction.Transaction.ValidationResult.*;
import static org.junit.Assert.*;
import static org.aquila.test.common.AccountUtils.fee;
...
public class SelfSponsorshipAlgoV1Tests extends Common {
	...
	public void testSingleSponsor() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			for (PrivateKeyAccount bobSponsee : bobSponsees)
				assertTrue(new Account(repository, bobSponsee.getAddress()).getConfirmedBalance(Asset.UNCIA) > 10); // 5 for transaction, 5 for fee
	...
	public void testMintBlockWithFounderSignerPenalty() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			for (PrivateKeyAccount aliceSponsee : aliceSponsees)
				assertTrue(new Account(repository, aliceSponsee.getAddress()).getConfirmedBalance(Asset.UNCIA) > 10); // 5 for transaction, 5 for fee
	...
	public void testDoubleTransferPrivs() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			// Bob and also sends some UNCIA to cover future transaction fees
