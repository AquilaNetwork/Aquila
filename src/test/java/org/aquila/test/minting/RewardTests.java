...
public class RewardTests extends Common {
	...
	public void testLevel1And2RewardsShareBinsV2() throws DataException {
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			AccountUtils.assertBalance(repository, "alice", Asset.UNCIA, aliceInitialBalance+expectedFounderRewardV2);
			AccountUtils.assertBalance(repository, "bob", Asset.UNCIA, bobInitialBalance); // Bob not online so his balance remains the same
			AccountUtils.assertBalance(repository, "chloe", Asset.UNCIA, chloeInitialBalance+expectedLevel1And2RewardV2);
			AccountUtils.assertBalance(repository, "dilbert", Asset.UNCIA, dilbertInitialBalance+expectedLevel1And2RewardV2);
			...
			AccountUtils.assertBalance(repository, "alice", Asset.UNCIA, aliceInitialBalance-expectedFounderRewardV1);
			AccountUtils.assertBalance(repository, "bob", Asset.UNCIA, bobInitialBalance); // Bob not online so his balance remains the same
			AccountUtils.assertBalance(repository, "chloe", Asset.UNCIA, chloeInitialBalance-expectedLevel1And2RewardV1);
			AccountUtils.assertBalance(repository, "dilbert", Asset.UNCIA, dilbertInitialBalance-expectedLevel1And2RewardV1);
			...
			// Validate balances
			AccountUtils.assertBalance(repository, "alice", Asset.UNCIA, aliceInitialBalance-(expectedFounderRewardV1*2));
			AccountUtils.assertBalance(repository, "bob", Asset.UNCIA, bobInitialBalance); // Bob not online so his balance remains the same
			AccountUtils.assertBalance(repository, "chloe", Asset.UNCIA, chloeInitialBalance-(expectedLevel1And2RewardV1*2));
			AccountUtils.assertBalance(repository, "dilbert", Asset.UNCIA, dilbertInitialBalance-(expectedLevel1And2RewardV1*2));
	...
	public void testLevel1And2RewardsShareBinsV2WithQoraHolders() throws DataException {
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			AccountUtils.assertBalance(repository, "alice", Asset.UNCIA, aliceInitialBalance+expectedFounderRewardV2);
			AccountUtils.assertBalance(repository, "bob", Asset.UNCIA, bobInitialBalance); // Bob not online so his balance remains the same
			// Chloe is a QORA holder and will receive additional UNCIA, so it's not easy to pre-calculate her balance
			AccountUtils.assertBalance(repository, "dilbert", Asset.UNCIA, dilbertInitialBalance+expectedLevel1And2RewardV2);
