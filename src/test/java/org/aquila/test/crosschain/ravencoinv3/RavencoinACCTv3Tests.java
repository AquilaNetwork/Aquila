package org.aquila.test.crosschain.ravencoinv3;
...
import org.aquila.crosschain.RavencoinACCTv3;
...
public class RavencoinACCTv3Tests extends Common {
	...
	public void testCompile() {
		...
		byte[] creationBytes = RavencoinACCTv3.buildAquilaAT(tradeAccount.getAddress(), ravencoinPublicKeyHash, redeemAmount, ravencoinAmount, tradeTimeout);
	...
	public void testTradingInfoProcessing() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			// Check trade partner Aquila address was extracted correctly
			assertEquals(partner.getAddress(), tradeData.aquilaPartnerAddress);
	...
	private DeployAtTransaction doDeploy(Repository repository, PrivateKeyAccount deployer, String tradeAddress) throws DataException {
		byte[] creationBytes = RavencoinACCTv3.buildAquilaAT(tradeAddress, ravencoinPublicKeyHash, redeemAmount, ravencoinAmount, tradeTimeout);
		...
		Long fee = null;
		String name = "UNCIA-RVN cross-chain trade";
		String description = String.format("Aquila-Ravencoin cross-chain trade");
		String atType = "ACCT";
		String tags = "UNCIA-RVN ACCT";
	...
	private void describeAt(Repository repository, String atAddress) throws DataException {
		...
		if (tradeData.mode != AcctMode.OFFERING && tradeData.mode != AcctMode.CANCELLED) {
			System.out.println(String.format("\trefund timeout: %d minutes,\n"
					...
					tradeData.aquilaPartnerAddress,
					tradeData.aquilaPartnerReceivingAddress));
		}
	}
